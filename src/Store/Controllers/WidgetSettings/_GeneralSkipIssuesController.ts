...
export default class GeneralSkipIssuesController
    implements IInputPropertyController
{
    ...
    get value(): string {
        return this._generalProperties.skipIssues?.toString() as string;
    }
    ...
    set value(v: string) {
        ...
        isNaN(nv) ? (nv = 0) : nv;
        if (nv >= 0) {
            this._generalProperties.skipIssues = nv;
        }
    }
