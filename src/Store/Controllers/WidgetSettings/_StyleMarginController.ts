...
export default class StyleMarginController
    implements ISpacingPropertyController
{
    ...
    set advanced(v: boolean) {
        ...
        if (match) {
            ...
            if (v && !right) {
                this._styleProperties.margin = `${top}px ${top}px ${top}px ${top}px`;
            } else if (!v && right) {
                this._styleProperties.margin = `${top}px`;
            }
        }
    }
    ...
    set value(
        ...
        if (typeof v === 'number') {
            this._styleProperties.margin = `${isValidNumber(v) ? v : 0}px`;
        }
        ...
        if (typeof v !== 'number') {
            ...
            this._styleProperties.margin = `${paddingValues.top}px ${paddingValues.right}px ${paddingValues.bottom}px ${paddingValues.left}px`;
        }
    }
