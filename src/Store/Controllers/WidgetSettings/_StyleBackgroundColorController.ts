...
export default class StyleBackgroundColorController implements IColorPropertyController {
    ...
    constructor(
        ...
        makeAutoObservable<StyleBackgroundColorController, '_domain' | '_uiState' | '_styleProperties'>(this, {
            _domain: false,
            _uiState: false,
        ...
        this._color = convert.hex.hsv.raw(this._styleProperties.backgroundColor as string);
    ...
    private initializeColorAndAlpha(): void {
        const backgroundColor = this._styleProperties.backgroundColor as string;
    ...
    private updateBackgroundColor(): void {
        if (this._alpha === 0) {
          this._styleProperties.backgroundColor = "transparent";
        } else {
          const [r, g, b] = convert.hsv.rgb.raw(this._color);
          // The alpha value is a percentage, so append '%' to the end.
          this._styleProperties.backgroundColor = `rgb(${Math.round(
            r
          )} ${Math.round(g)} ${Math.round(b)} / ${this._alpha}%)`;
        }
    }
