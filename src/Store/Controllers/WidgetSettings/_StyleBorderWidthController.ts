...
export default class StyleBorderWidthController implements ISliderPropertyController {
    ...
    constructor(
        ...
        makeAutoObservable<StyleBorderWidthController, '_domain' | '_uiState' | '_styleProperties'>(this, {
            _domain: false,
            _uiState: false,
    ...
    get value(): number {
        const match = /(?<value>\d+)px/.exec(this._styleProperties.borderWidth as string);
        if (match) {
    ...
    set value(v: number) {
        this._styleProperties.borderWidth = `${v}px`;
    }
