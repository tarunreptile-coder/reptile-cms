...
type IThemesMarginController = Reptile.Controllers.IThemesMarginController;

export default class ThemesMarginController
    implements IThemesMarginController
{
    ...
    set advanced(v: boolean) {
        ...
        if (match) {
            ...
            if (v && !right) {
                this.theme?.update(this._selectedRule, this._property, `${top}px ${top}px ${top}px ${top}px`)
            } else if (!v && right) {
                this.theme?.update(this._selectedRule, this._property, `${top}px`)
            }
        }
    }
    ...
    set value(
        ...
        if (typeof v === 'number') {
            this.theme?.update(this._selectedRule, this._property, `${isValidNumber(v) ? v : 0}px`)
        }
        ...
        if (typeof v !== 'number') {
            ...
            this.theme?.update(this._selectedRule, this._property, `${paddingValues.top}px ${paddingValues.right}px ${paddingValues.bottom}px ${paddingValues.left}px`)
        }
    }
